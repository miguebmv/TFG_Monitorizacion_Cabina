...
int PWMPin = 14;
//A continuacion establecemos la temperatura idonea
float TempQuerida = 21.5;
//Creamos la constante que ira de 0 a 255 para controlar dicha señal
int PWMvalor=0;
//-----------------------------------------------------
...
if (ahora - ultimo_mensaje >= 25000) {
    ...
    //Ahora vamos a implementar la moficiación de la señal PWM dependiendo de la temperatura que recibimos
    float diferencia = datos.temperatura-TempQuerida;
    Serial.print("Diferencia TEMP:   "); Serial.print(diferencia); Serial.println(" ºC");
    if (diferencia>0)
    {
      if (diferencia>=4)
      {
      ...
      {
      PWMvalor= (diferencia*255)/(4);
      }
      analogWrite(PWMPin, PWMvalor);
    }
    Serial.print("PWM:   "); Serial.println(PWMvalor);
